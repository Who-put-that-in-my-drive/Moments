...
describe('/user - User API endpoint', () => {
    ...
    it('Incorrect First Name For Update User API Request', async () => {
        let result = await request(app).put(userApiRoute).send({firstName: faker.datatype.string(0), lastName: validUpdateUserDTO.lastName}).set('Authorization', jwt);
        expect(result.body.msg).toEqual('First Name Under/Exceeds Character Length');
        expect(result.statusCode).toEqual(400);

        result = await request(app).put(userApiRoute).send({firstName: faker.datatype.string(19), lastName: validUpdateUserDTO.lastName}).set('Authorization', jwt);
        expect(result.body.msg).toEqual('First Name Under/Exceeds Character Length');
        expect(result.statusCode).toEqual(400);
    });
    it('Incorrect Last Name For Update User API Request', async () => {
        let result = await request(app).put(userApiRoute).send({firstName: validUpdateUserDTO.firstName, lastName: faker.datatype.string(0)}).set('Authorization', jwt);
        expect(result.body.msg).toEqual('Last Name Under/Exceeds Character Length');
        expect(result.statusCode).toEqual(400);

        result = await request(app).put(userApiRoute).send({firstName: validUpdateUserDTO.firstName, lastName: faker.datatype.string(19)}).set('Authorization', jwt);
        expect(result.body.msg).toEqual('Last Name Under/Exceeds Character Length');
        expect(result.statusCode).toEqual(400);
    });
    afterAll(async () => {
